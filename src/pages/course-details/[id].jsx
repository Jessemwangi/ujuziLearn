...
const DynamicCourseDetails = () => {
    ...
    useEffect(() => {
        // Wait for router to be ready and ensure both id and docId are available
        if (router.isReady && id && docId) {
            const fullUrl = `/courses/${docId}${query}`;
            // console.log('Dispatching fetchCourse with URL:', fullUrl);
            // console.log('Route ID:', id, 'Doc ID:', docId);
            dispatch(fetchCourse(fullUrl));
        }
    }, [id, docId, router.isReady, dispatch, query]);

    // console.log('Route params - ID:', id, 'DocId:', docId);
        ...
        return (
            <Wrapper>
                ...
                <div style={{ textAlign: 'center', padding: '80px 0' }}>
                    ...
                    <p>There was an error loading the course details. Please try again later.</p>
