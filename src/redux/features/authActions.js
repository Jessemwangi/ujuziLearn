...
export const login = (apiUrl, object) => async (dispatch) => {
  try {
    ...
    } else if (response.user && response.sessionInfo) {
      // Store the entire sessionInfo object
      TokenService.setUser(response.user);
      TokenService.setToken(response.sessionInfo); 
      
      dispatch(setUser(response.user));
      dispatch(setToken(response.sessionInfo)); 
